...
for (i in 2:14) {
  ...
  Boston.lm.coefficients[i - 1] <- coef(lm.fit)[1]
...
```
    ...
    Logistic regression does an admirable job in classifying the crime rate, with an overall accurate of 87.3% using a training and test set. From the predictions, it could be seen that the main error comes from reporting a high crime rate as a low crime rate, thus lowering the specificity. In other words, if we define having a higher crime rate as the positive class, logistic regression tends to report false negative, thus making a type 2 error. Predictors nox and rad are shown to be statistically significant at the p < 0.001 level, while age and medv are statistically significant at the p < 0.01 level. ptratio, which was previously shown to be statistically significant in linear regression, is no longer shown to be statistically significant in logistic regression.
...
```
    ...
    Using cross-validation, k = 3 was selected for the kNN model. Testing using the test dataset revealed an accuracy of 93.65%, which is still superior to that of logistic regression of 87.3% above. However, performance between the two models are indistinguishable at the p < 0.05 level, as shown by the overlap by the respective 95% CI.
...
```
    ...
    The naive Bayes model performed in a manner slightly worse than the logistic regression model, and worse than the kNN model. Similarly to the logistic regression model, it tends to classify a high crime rate as a low crime rate.
...
```
    ...
    By bootstrapping with 1000 replication, we obtain a similar coefficient of the medv predictor. The standard error from bootstrapping is reported to be 0.00188, which is lower than the asymptotic standard error from the maximum likelihood of 0.00224. These values should be sufficiently close for use, and bootstrapping would be useful when maximum likelihood estimate (MLE) is not available. Increasing the number of replications would result in the standard error asymptotically approaching that given by MLE.
